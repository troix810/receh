...
contract WRCH is ITokenDeposit {
    string public name = "Wrapped RCH";
    string public symbol = "WRCH";
    uint8  public decimals = 6;
    trcToken  public rchTokenId = trcToken(1003919);
    ...
    function deposit() public payable {
        require(msg.tokenid == rchTokenId, "deposit tokenId not RCH[1003919]");
    ...
    function withdraw(uint256 sad) public {
        require(balanceOf_[msg.sender] >= sad, "not enough WRCH balance");
        require(totalSupply_ >= sad, "not enough WRCH totalSupply");
        balanceOf_[msg.sender] -= sad;
        totalSupply_ -= sad;
        msg.sender.transferToken(sad, rchTokenId);
